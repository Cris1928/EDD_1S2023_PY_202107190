...
class AvlTree{
    ...
    search(nombre, password){
        return this.#searchRecursive(nombre, password, this.root);
    }
        ...
        if(node == null){
            ...
            if(this.getHeight(node.left) - this.getHeight(node.right) == 2){
                if(item.carnet < node.left.item.carnet){
                  ...
                  //  console.log("Rotación simple a la izquierda");
                }else{
    ...
    #inOrderRecursive(current){
        ...
        row +=`
            <tr>
                <th style="color:black">${current.item.carnet}</th>
                <td style="color:black">${current.item.nombre}</td>
                <td style="color:black">${current.item.password}</td>
            </tr>
        `;
        if(current.right != null){
    ...
    #searchRecursive(name, pass, node){
        if(node == null){
            return false;
        }else if(name == node.item.nombre && pass == node.item.password){
            return true;
        }if(node.right != null){
            return this.#searchRecursive(name, pass, node.left);
        }

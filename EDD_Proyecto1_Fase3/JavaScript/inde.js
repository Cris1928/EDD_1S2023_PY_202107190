// USUARIOS DE PRUEBA PARA LOS MENSAJES
const users = [
     {
            nombre: "Hugo Rosal",
            carnet: 8318054,
            password: "Hugo30",
            Carpeta_Raiz: "/"
        },
        {
            nombre: "Luis Pirir",
            carnet: 9616453,
            password: "Luis32",
            Carpeta_Raiz: "/"
        },
        {
            nombre: "Williams Constanza",
            carnet: 199919737,
            password: "Williams42",
            Carpeta_Raiz: "/"
        },
        {
            nombre: "Jim Melendez",
            carnet: 200715321,
            password: "Jim67",
            Carpeta_Raiz: "/"
        },
        {
            nombre: "William Ambrocio",
            carnet: 201403669,
            password: "William29",
            Carpeta_Raiz: "/"
        },
        {
            nombre: "Ebany Larios",
            carnet: 201403877,
            password: "qwerty12",
            Carpeta_Raiz: "/"
        },
        {
            nombre: "Helber Urias",
            carnet: 201404028,
            password: "Helber67",
            Carpeta_Raiz: "/"
        },
        {
            nombre: "Manolo Ramirez",
            carnet: 201503933,
            password: "Manolo93",
            Carpeta_Raiz: "/"
        },
        {
            nombre: "Jose Boguerin",
            carnet: 201544815,
            password: "Jose50",
            Carpeta_Raiz: "/"
        },
        {
            nombre: "Kevin Secaida",
            carnet: 201602404,
            password: "Kevin46",
            Carpeta_Raiz: "/"
        },
        {
            nombre: "Bryan Ignacio",
            carnet: 201602516,
            password: "Bryan75",
            Carpeta_Raiz: "/"
        },
        {
            nombre: "Oscar Morales",
            carnet: 201603028,
            password: "Oscar59",
            Carpeta_Raiz: "/"
        },
        {
            nombre: "Alejandro Yac",
            carnet: 201700315,
            password: "Alejandro34",
            Carpeta_Raiz: "/"
        },
        {
            nombre: "Dennis Escobar",
            carnet: 201700792,
            password: "Dennis28",
            Carpeta_Raiz: "/"
        },
        {
            nombre: "Alma Manchame",
            carnet: 201700907,
            password: "Alma28",
            Carpeta_Raiz: "/"
        },
        {
            nombre: "Cristian Suy",
            carnet: 201700918,
            password: "Cris123",
            Carpeta_Raiz: "/"
        },
        {
            nombre: "Kevin Sotoj",
            carnet: 201710130,
            password: "Kevin80",
            Carpeta_Raiz: "/"
        },
        {
            nombre: "Leonardo Martinez",
            carnet: 201780044,
        password: "leo123",
            Carpeta_Raiz: "/"
        },
        {
            nombre: "Wilmer Vasquez",
            carnet: 201800678,
            password: "Wilmer27",
            Carpeta_Raiz: "/"
        },
        {
            nombre: "Erick Chacon",
            carnet: 201807169,
            password: "Erick91",
            Carpeta_Raiz: "/"
        },
        {
            nombre: "Brian Garcia ",
            carnet: 201807351,
            password: "Brian12",
            Carpeta_Raiz: "/"
        },
        {
            nombre: "Gladys Ajuchan",
            carnet: 201807389,
            password: "Gladys17",
            Carpeta_Raiz: "/"
        },
        {
            nombre: "Erick Gonzalez",
            carnet: 201900621,
            password: "Erick81",
            Carpeta_Raiz: "/"
        },
        {
            nombre: "Kevin Castaneda",
            carnet: 201901801,
            password: "Kevin3",
            Carpeta_Raiz: "/"
        },
        {
            nombre: "Christopher Monterroso",
            carnet: 201902363,
            password: "Christopher66",
            Carpeta_Raiz: "/"
        },
        {
            nombre: "Javier Pablo",
            carnet: 201944766,
            password: "Javier99",
            Carpeta_Raiz: "/"
        },
        {
            nombre: "Robin Buezo",
            carnet: 201944994,
            password: "Robin27",
        Carpeta_Raiz: "/"
        },
        {
            nombre: "Moises Conde",
            carnet: 202000127,
            password: "Moises18",
            Carpeta_Raiz: "/"
        },
        {
            nombre: "Diana Berducido",
            carnet: 202000277,
            password: "Diana55",
            Carpeta_Raiz: "/"
        },
        {
            nombre: "Javier Matias",
            carnet: 202000896,
            password: "Javier47",
            Carpeta_Raiz: "/"
        },
        {
            nombre: "Kevin Hernandez",
            carnet: 202001053,
            password: "Kevin4",
            Carpeta_Raiz: "/"
        },
        {
            nombre: "Nataly Guzman",
            carnet: 202001570,
            password: "Nataly84",
            Carpeta_Raiz: "/"
        },
        {
            nombre: "Angela Pinelo",
            carnet: 202002536,
            password: "Angela92",
            Carpeta_Raiz: "/"
        },
        {
            nombre: "Jorge Zamora",
            carnet: 202002591,
            password: "Jorge33",
            Carpeta_Raiz: "/"
        },
        {
            nombre: "Gerber Ordonez",
            carnet: 202004060,
            password: "Gerber30",
            Carpeta_Raiz: "/"
        },
        {
            nombre: "Fredy Quijada",
            carnet: 202004812,
            password: "Fredy63",
            Carpeta_Raiz: "/"
        },
        {
            nombre: "Luis Moreira",
            carnet: 202010770,
            password: "Luis55",
            Carpeta_Raiz: "/"
        },
        {
            nombre: "Luis Garcia",
            carnet: 202010814,
            password: "Luis39",
            Carpeta_Raiz: "/"
        },
        {
            nombre: "Maria Tebalan",
            carnet: 202100265,
            password: "Maria41",
            Carpeta_Raiz: "/"
        },
        {
            nombre: "Damaris Muralles",
            carnet: 202100953,
            password: "Damaris89",
            Carpeta_Raiz: "/"
        },
        {
            nombre: "Denis Coronado",
            carnet: 202101499,
            password: "Denis78",
            Carpeta_Raiz: "/"
        },
        {
            nombre: "Steven Mejia",
            carnet: 202104160,
            password: "Steven18",
            Carpeta_Raiz: "/"
        },
        {
            nombre: "Wendi Vicente",
            carnet: 202106484,
            password: "Wendi52",
            Carpeta_Raiz: "/"
        },
        {
            nombre: "Pablo Barahona ",
            carnet: 202109715,
            password: "Pablo42",
            Carpeta_Raiz: "/"
        },
        {
            nombre: "Jose Galdamez",
            carnet: 202109732,
            password: "Jose21",
            Carpeta_Raiz: "/"
        },
        {
            nombre: "Aldo Vasquez",
            carnet: 202109754,
            password: "Aldo65",
            Carpeta_Raiz: "/"
        }
]
//console.log("esto debe de mostrarse solo una vez");
//console.log(users);}
// MOSTRAR LOS USAURIOS EN LOS SELECTS
$(document).ready(() => {
    console.log("esto debe de mostrarse solo una vez");
    console.log(window.Lusers);
    let optionsForSelect1 = "";
    let optionsForSelect2 = "";
console.log("esto debe de mostrarse solo una vez 2");
    console.log(window.Lusers.length);
    console.log("esto debe de mostrarse solo una vez 3");


    users.forEach( (user, i) => {
       // console.log("user: ", user);
        if(user.carnet !==window.userObtenido){
          //  optionsForSelect1 += `
    //            <option value="${user.carnet}">${user.nombre}</option>
 //           `;    
   //     }else{
   // console.log("user: ", user.carnet);
           optionsForSelect2 += `
                <option value="${user.carnet}">${user.nombre}</option>
            `;
        }
    });
   // optionsForSelect1 = `<option value="${ window.userObtenido}">"cualquiera"</option>`
    $('#transmitter').append(optionsForSelect1);
    $('#receiver').append(optionsForSelect2);

});

// INSTANCIA DE LA CLASE
let blockChain = new BlockChain();


// ACTUALIZAR AMBOS CHATS 
function updateChats(){
    let transmitter = window.userObtenido;
    //let transmitter = $('#transmitter').val();


    console.log("Transmitter: ", typeof transmitter);
    console.log( window.userObtenido );
    console.log("el que quiero: ",typeof window.userObtenido );
    let receiver = $('#receiver').val();
    $('#transmitter-chat').html(blockChain.getMessages(transmitter, receiver));
    $('#receiver-chat').html(blockChain.getMessages(receiver, transmitter));
}


async function sendMessage(whoSend){
    // OBTENER VALORES DEL SELECT 
  // let transmitter = $('#transmitter').val();
  let transmitter = window.userObtenido;
    let receiver = $('#receiver').val();
    
    // VERIFICAR QUE HAYA SELECCIONADO UN USUARIO
    if(transmitter && receiver){
        switch(whoSend){
            case 'transmitter':
                // OBTENER MENSAJE A ENVIAR
                let msgt = $('#msg-transmitter').val();
                // INSERTAR MENSAJE EN BLOCKCHAIN
                await blockChain.insert(transmitter, receiver, msgt);
                $('#msg-transmitter').val("");
            break;
            case 'receiver':
                // OBTENER MENSAJE A ENVIAR
                let msgr = $('#msg-receiver').val();
                // INSERTAR MENSAJE EN BLOCKCHAIN
                await blockChain.insert(receiver, transmitter, msgr);
                $('#msg-receiver').val("");
            break;
        }
        alert("Mensaje enviado");
        // ACTUALIZAR CHATS
        updateChats();
    }else{
        alert("No ha seleccionado Receptop o Emisor");
    }
    window.listaPersonas.limpiarLista();
     blockChain.llenar();

}


function getBlock(index){
    if(index === 0){
        let html = blockChain.blockReport(index);
        if(html){
            $('#show-block').html(html);
        }
    }else{
        let currentBlock = Number($('#block-table').attr('name'));
        
        if(index < 0){ // MOSTRAR EL ANTERIOR
            if(currentBlock - 1 < 0){
                alert("No existen elementos anteriores");
            }else{
                let html = blockChain.blockReport(currentBlock - 1);
                if(html){
                    $('#show-block').html(html);
                }
            }

        }else if(index > 0){ // MOSTRAR EL SIGUIENTE
            if(currentBlock + 1 > blockChain.size ){
                alert("No existen elementos siguientes");
            }else{
                let html = blockChain.blockReport(currentBlock + 1);
                if(html){
                    $('#show-block').html(html);
                }
            }
        }
    }
}



